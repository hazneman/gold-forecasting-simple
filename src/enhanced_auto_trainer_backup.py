...
class EnhancedAutoMLTrainer:
    ...
    def train_extended_models_with_progress(self, period="3y", force_retrain=False):
        """Train models for all horizons with progress tracking and intelligent caching"""
        try:
            ...
            # Initialize progress
            total_steps = len(self.horizons) + 5  # Data collection, feature engineering, validation, saving, cleanup
            self.update_progress("🔍 Checking for existing models...", 0, "Initializing intelligent caching system...")
            
            # Check if models already exist and are recent
            if not force_retrain:
                models_exist, status_message = self.check_models_exist()
                logger.info(f"Model cache status: {status_message}")
                
                if models_exist:
                    # Try incremental update first
                    new_data = self.get_new_data_since_last_update()
                    
                    if new_data is None or new_data.empty:
                        # No new data, load existing models
                        self.update_progress("✅ Using cached models - no new data found", 95, "Loading cached models...")
                        elapsed_time = time.time() - start_time
                        logger.info(f"🎯 Model loading completed in {elapsed_time:.1f} seconds (cached)")
                        ...
                        if results_file.exists():
                            ...
                            self.update_progress("✅ Cached models loaded successfully!", 100, f"Loaded {len(cached_results)} cached models in {elapsed_time:.1f}s")
                            self.training_progress['status'] = 'completed'
                            self.is_training = False
                            return {
                                'status': 'success',
                                'timestamp': datetime.now().isoformat(),
                                'models_trained': len(cached_results),
                                'horizons': list(cached_results.keys()),
                                'summary': f"Loaded {len(cached_results)} cached models instantly",
                                'cached': True,
                                'training_time': elapsed_time
                            }
                    else:
                        # Perform incremental update
                        logger.info(f"🔄 Performing incremental update with {len(new_data)} new data points")
                        self.update_progress("📊 Incremental update", 1, f"Found {len(new_data)} new data points")
                        return self._perform_incremental_update(new_data, start_time)
            
            # Full training needed
            self.update_progress("📊 Collecting training data...", 5, "Downloading fresh market data...")
            ...
            if cached_data is not None and not force_retrain:
                ...
                data = cached_data
                self.update_progress("📊 Using cached training data", 15, f"Loaded {len(data)} cached samples")
            else:
                logger.info("📊 Collecting fresh training data")
                data = self.collect_extended_training_data(period=period)
                if data is None or data.empty:
                    raise Exception("Failed to collect training data")
                
                # Cache the training data
                self.save_training_data_cache(data)
                self.update_progress("📊 Data collection complete", 15, f"Collected {len(data)} fresh samples")
            ...
            # Data validation
            self.update_progress("✅ Validating data quality...", 36, "Running quality checks...")
            validation_results = self.validate_data_quality(features_df)
            logger.info(f"✅ Data validation completed: {validation_results}")
            
            # Train models for each horizon
            self.training_progress['total_models'] = len(self.horizons)
            X = features_df[feature_cols].fillna(method='ffill').fillna(method='bfill')
            results = {}
            
            for i, horizon in enumerate(self.horizons):
                horizon_name = self.horizon_names[horizon]
                model_progress = 37 + (i / len(self.horizons)) * 55
                
                self.update_progress(
                    f"🤖 Training {horizon_name} models ({i+1}/{len(self.horizons)})...",
                    model_progress,
                ...
                # Create target
                y = (features_df['Close'].shift(-horizon) / features_df['Close'] - 1) * 100
                ...
                try:
                    model_result = self.train_horizon_models(features_df, horizon, horizon_name)
                    results[horizon_name] = model_result
                    
                    self.update_progress(
                        f"✅ {horizon_name} model completed",
                        model_progress,
                        f"Accuracy: {model_result.get('accuracy', 'N/A'):.1f}%"
                    )
                    logger.info(f"🎯 {horizon_name} model completed: {model_result['best_algorithm']} "
                              f"(Accuracy: {model_result.get('accuracy', 0):.1f}%)")
                except Exception as e:
                    self.update_progress(
            ...
            # Save results and update cache metadata
            self.update_progress("💾 Saving models and results...", 95, "Serializing models to disk...")
            self.save_results(results)
            
            # Update cache metadata
            cache_metadata = {
                'trained_models': {name: result['best_algorithm'] for name, result in results.items()},
                ...
                'model_performance': {name: result.get('accuracy', 0) for name, result in results.items()}
            }
            self.save_cache_metadata(cache_metadata)
            ...
            self.update_progress(
                "✅ Training complete!",
                100,
                f"Successfully trained {len(results)} models in {elapsed_time:.1f}s"
            )
            
            logger.info(f"🎯 Extended model training completed in {elapsed_time:.1f} seconds")
            self.training_progress['status'] = 'completed'
            self.is_training = False
            ...
            return {
                ...
                'summary': f"Trained {len(results)} models with {len(feature_cols)} features",
                'cached': False,
                'training_time': elapsed_time
            }
    ...
    def _perform_incremental_update(self, new_data, start_time):
        """Perform incremental model update with new data"""
        try:
            # Engineer features for new data
            self.update_progress("🔧 Engineering features for new data...", 20, f"Processing {len(new_data)} new samples...")
            new_features = self.engineer_features(new_data)
            
            # Validate new data
            self.update_progress("✅ Validating new data...", 25, "Running quality checks on new data...")
            validation_results = self.validate_data_quality(new_features)
            
            # Load existing models and update them
            self.training_progress['total_models'] = len(self.horizons)
            results = {}
            
            # Load cached training data and combine with new data
            cached_data = self.load_cached_training_data()
            if cached_data is not None:
                # Combine old and new data
                combined_data = pd.concat([cached_data, new_data]).drop_duplicates()
                combined_features = self.engineer_features(combined_data)
                logger.info(f"🔄 Combined data: {len(cached_data)} cached + {len(new_data)} new = {len(combined_features)} total")
            else:
                combined_features = new_features
                logger.info(f"🔄 Using only new data: {len(new_features)} samples")
            
            # Prepare features for training
            feature_cols = [col for col in combined_features.columns 
                           if col not in ['Open', 'High', 'Low', 'Close', 'Adj Close', 'Volume'] 
                           and combined_features[col].dtype in ['float64', 'int64']]
            
            X = combined_features[feature_cols].fillna(method='ffill').fillna(method='bfill')
            
            for i, horizon in enumerate(self.horizons):
                horizon_name = self.horizon_names[horizon]
                model_progress = 30 + (i / len(self.horizons)) * 60
                
                self.update_progress(
                    f"🔄 Updating {horizon_name} model ({i+1}/{len(self.horizons)})...",
                    model_progress,
                    f"Incremental training for {horizon_name}..."
                )
                self.training_progress['current_model'] = horizon_name
                self.training_progress['models_completed'] = i
                
                # Create target
                y = (combined_features['Close'].shift(-horizon) / combined_features['Close'] - 1) * 100
                ...
                try:
                    model_result = self.train_horizon_models(combined_features, horizon, horizon_name)
                    results[horizon_name] = model_result
                    
                    self.update_progress(
                        f"✅ {horizon_name} updated",
                        model_progress,
                        f"Accuracy: {model_result.get('accuracy', 'N/A'):.1f}%"
                    )
                    logger.info(f"🔄 {horizon_name} model updated: {model_result['best_algorithm']} "
                              f"(Accuracy: {model_result.get('accuracy', 0):.1f}%)")
                except Exception as e:
                    self.update_progress(
                        f"❌ {horizon_name} update failed",
                        model_progress,
                        f"Error: {str(e)}"
                    )
                    logger.error(f"Failed to update {horizon_name}: {e}")
            
            # Save results and update cache
            self.update_progress("💾 Saving updated models...", 95, "Updating model cache...")
            self.save_results(results)
            
            # Update training data cache with combined data
            if 'combined_data' in locals():
                self.save_training_data_cache(combined_data)
            ...
            cache_metadata.update({
                ...
                'training_duration_minutes': (time.time() - start_time) / 60,
                'incremental_samples': len(new_data),
                'validation_results': validation_results,
                'model_performance': {name: result.get('accuracy', 0) for name, result in results.items()}
            })
            self.save_cache_metadata(cache_metadata)
            ...
            self.update_progress(
                ...
                100,
                f"Updated {len(results)} models in {elapsed_time:.1f}s"
            )
            
            logger.info(f"🔄 Incremental model update completed in {elapsed_time:.1f} seconds")
            self.training_progress['status'] = 'completed'
            self.is_training = False
            ...
            return {
                ...
                'models_trained': len(results),
                'horizons': list(results.keys()),
                'summary': f"Updated {len(results)} models with {len(new_data)} new samples",
                'incremental': True,
                'training_time': elapsed_time
            }
    ...
    def save_results(self, results):
        ...
        try:
            ...
            for horizon_name, result in results.items():
                if 'model' in result:
                    model_file = self.model_dir / f"gold_model_{horizon_name}.pkl"
                    joblib.dump(result['model'], model_file)
                    logger.info(f"💾 Saved model: {model_file}")
            
            logger.info(f"✅ Results saved: {len(results)} models")
            
        except Exception as e:
            logger.error(f"Failed to save results: {e}")
    
    def train_horizon_models(self, features_df, horizon, horizon_name):
        """Train models for a specific horizon"""
        try:
            # Determine which algorithms to use based on horizon
            if horizon <= 14:
                algorithms = self.algorithms['short_term']
            elif horizon <= 90:
                algorithms = self.algorithms['medium_term']
            ...
            if len(X_clean) < 200:
                logger.warning(f"Insufficient data for {horizon_name}: {len(X_clean)} samples")
                return {
                    'horizon_days': horizon,
                    'horizon_name': horizon_name,
                    'best_algorithm': 'insufficient_data',
                    'best_score': 0.0,
                    'accuracy': 0.0,
                    'samples': len(X_clean)
                }
            ...
            result = {
                'horizon_days': horizon,
                'horizon_name': horizon_name,
                'best_algorithm': best_algorithm or 'none',
            ...
            logger.error(f"Failed to train models for {horizon_name}: {e}")
            return {
                'horizon_days': horizon,
                'horizon_name': horizon_name,
